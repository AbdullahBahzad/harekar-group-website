...
export default function ReportConsole({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ---- recipients ----------------------------------------------------- */}
      <Panel label={t("reports.recipients")}>
        <div className="text-bone/55 flex flex-wrap items-center gap-x-4 gap-y-2 p-4 text-sm leading-relaxed">
          <p className="min-w-0 flex-1">{t("reports.recipientsIntro")}</p>
          <Link
            href="/admin/emailing"
            className="border-gold/40 text-gold hover:bg-gold hover:text-ink flex min-h-11 shrink-0 items-center border px-3 text-xs transition-colors"
          >
            {t("reports.recipientsOpen")}
          </Link>
        </div>
      </Panel>
